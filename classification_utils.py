...
class SequenceClassificationModel:
    def __init__(self, model_type, model_name, num_labels=None, weight=None, args=None, use_cuda=True, cuda_device=-1):
        ...
        if model_type in ['camembert', 'xlmroberta']:
            warnings.warn(
                ...
                f"fails when using multiprocessing for feature conversion.")
    ...
    def train_model(self, train_df, multi_label=False, output_dir=None, show_running_loss=True, args=None, eval_df=None,
        ...
        if 'text' in train_df.columns and 'labels' in train_df.columns:
            ...
            train_examples = [InputExample(i, text_a, text_b, label) for i, (text_a, text_b, label) in
                              enumerate(zip(train_df['text_a'], train_df['text_b'], train_df['labels']))]
        else:
            warnings.warn(
        ...
        global_step, tr_loss = self.train(train_dataset, output_dir, multi_label=multi_label,
                                          show_running_loss=show_running_loss, eval_df=eval_df, **kwargs)
            ...
            for step, batch in enumerate(tqdm(train_dataloader,
                ...
                if (step + 1) % args["gradient_accumulation_steps"] == 0:
                    ...
                    if args["logging_steps"] > 0 and global_step % args["logging_steps"] == 0:
                        ...
                        if args['wandb_project']:
                            wandb.log({'Training loss': current_loss, 'lr': scheduler.get_lr()[0],
                    ...
                    if args['evaluate_during_training'] and (
                            args["evaluate_during_training_steps"] > 0 and global_step % args[
    ...
    def evaluate(self, eval_df, output_dir, multi_label=False, prefix="", **kwargs):
        ...
        if 'text' in eval_df.columns and 'labels' in eval_df.columns:
            ...
            eval_examples = [InputExample(i, text_a, text_b, label) for i, (text_a, text_b, label) in
                             enumerate(zip(eval_df['text_a'], eval_df['text_b'], eval_df['labels']))]
        else:
            warnings.warn(
                "Dataframe headers not specified. Falling back to using column 0 as text and column 1 as labels.")
            eval_examples = [InputExample(i, text, None, label) for i, (text, label) in
        ...
        if args['sliding_window']:
            ...
            out_label_ids = [out_label_ids[i] for i in range(len(out_label_ids)) if
                             i in [window[0] for window in window_ranges]]
    ...
    def load_and_cache_examples(self, examples, evaluate=False, no_cache=False, multi_label=False):
        ...
        if os.path.exists(cached_features_file) and ((not args["reprocess_input_data"] and not no_cache) or (
        ...
        if args['sliding_window']:
            ...
            if multi_label:
                if isinstance(args['threshold'], list):
                    ...
                    preds = [[self._threshold(pred, threshold_values[i]) for i, pred in enumerate(example)] for example
                             in preds]
                else:
